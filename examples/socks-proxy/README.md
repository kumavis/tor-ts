...
A runnable Node.js demo that bootstraps a 3-hop Tor circuit on the live
network and exposes it as a SOCKS5 proxy on `localhost:1080`. Any
SOCKS5-aware client (curl, browsers, scripts, language SDKs) can
plug in without linking against `tor`.
...
# A SOCKS RESOLVE (proposal 100) — anonymous DNS over the circuit:
...
3. `USERNAME_PASSWORD` (RFC 1929) is accepted as a stream-isolation
   key per c-tor / Arti convention. The default config doesn't act
   on it — see `circuitFactory` in `SocksProxyServer` for how to
   give each isolation key its own circuit.

## Code

See `index.ts`. Around 100 lines, including signal-handling and
diagnostics.
