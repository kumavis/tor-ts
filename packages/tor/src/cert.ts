...
export function parseEd25519Certificate(certBody: Buffer): Ed25519Certificate {
  ...
  const keyExtension = extensions.find(
    ({ type }) => type === ExtensionTypes.SIGNED_WITH_ED25519_KEY
  );
  let signedWith;
  if (keyExtension) {
...
export const validateCertsCellForIdentities = (
  ...
  assert(
    certs.filter(({ type }) => type === CertTypes.RSA_ID_X509).length === 1,
    'The CERTS cell contains exactly one CertType 2 "ID" certificate.'
  );
  // 	* The CERTS cell contains exactly one CertType 4 Ed25519
  // 		"Id->Signing" cert.
  assert(
    certs.filter(({ type }) => type === CertTypes.IDENTITY_V_SIGNING).length === 1,
  ...
  assert(
    certs.filter(({ type }) => type === CertTypes.SIGNING_V_TLS_CERT).length === 1,
  ...
  // 		cross-certificate.
  assert(
    certs.filter(({ type }) => type === CertTypes.RSA_ID_V_IDENTITY).length === 1,
...
function parseRsaCrossCertificate(certBody: Buffer): CrossCertificate {
  ...
  const digest = sha256(RsaCrossCertPrefix, signedPortion);
