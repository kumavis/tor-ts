/**
 * Browser-compatible hidden service (.onion) support.
 * Implements HSv3 client-side connection flow using Web Crypto.
 *
 * This module provides the ability to connect to .onion addresses via
 * the Tor network using the Snowflake browser channel.
 */

import { x25519, ed25519 } from '@noble/curves/ed25519';
import { sha3_256, shake256 } from '@noble/hashes/sha3';
import { Circuit } from 'tor/circuit';
import type { CircuitCipherPair, CopyableHash, PeerInfo } from 'tor/circuit';
import { RelayCell } from 'tor/relay-cell';
import { parseEd25519Certificate } from 'tor/cert';
import type { LinkSpecifier } from 'tor/messaging';
import { makeAes256CtrKey } from 'tor/aes';
import {
  DirectoryClient,
  lookupPeerInfo,
...
/**
 * Browser-compatible AES-256-CTR XOR using Web Crypto.
 * This replaces the Node.js crypto.createDecipheriv implementation.
 */
async function aes256CtrXor(key: Buffer, iv: Buffer, data: Buffer): Promise<Buffer> {
  // Import the key for AES-CTR
  ...
  // Encrypt/decrypt using AES-CTR (symmetric, so encrypt = decrypt)
  const result = await crypto.subtle.encrypt(
    {
      name: 'AES-CTR',
      counter: Uint8Array.from(iv),
      length: 64, // Counter length in bits (standard for AES-CTR)
    },
    cryptoKey,
...
/**
 * Parse a v3 .onion address and extract the public identity key.
 */
...
export function computeTimePeriod(params: {
  ...
  let votingIntervalSeconds = 3600;
...
// ============================================================================
// Descriptor parsing and decryption
// ============================================================================
...
// ============================================================================
// Introduction point types and parsing
// ============================================================================
...
function makeHsRendezvousCipherPairFromKeySeed(NTOR_KEY_SEED: Buffer): CircuitCipherPair {
  const PROTOID = Buffer.from('tor-hs-ntor-curve25519-sha3-256-1', 'ascii');
  const m_hsexpand = Buffer.from(`${PROTOID.toString('ascii')}:hs_key_expand`, 'ascii');
...
async function buildIntroduce1Payload(params: {
...
export async function connectToHiddenService(
  ...
  log('Decrypting hidden service descriptor...');
...
/**
 * Fetch hidden service descriptor via a directory stream.
 */
async function fetchHsDescriptorOverDirectoryStream(
  ...
  // Open a directory stream to the HSDir
  const stream = await bootstrapCircuit.openDirectoryStream();
