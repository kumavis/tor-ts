...
test('turbotunnel: buildTurbotunnelPreamble concatenates token and clientId (no padding)', (t) => {
  const id = Uint8Array.from([1, 2, 3, 4, 5, 6, 7, 8]);
  const pre = buildTurbotunnelPreamble(id, { padding: false });
...
test('turbotunnel: buildTurbotunnelPreamble rejects wrong clientId length', (t) => {
  t.throws(() => buildTurbotunnelPreamble(new Uint8Array(7), { padding: false }));
});
